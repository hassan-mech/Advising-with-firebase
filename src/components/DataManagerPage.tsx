 ...
 * DataManagerPage — v3.
 *
 * Full page (not a modal) for managing the three datasets:
 * Grade-book / Catalog / Roster. Reached via the "Manage data" button
 * in ImportButtons, which calls onChangeView('manage') in App.tsx —
 * same navigation pattern PrereqMapScreen uses (onBack callback, no
 * router).
 *
 * Each tab exposes:
 ...
 *
 * BUGFIX (v3): `renderRow` now receives `beginEdit` as a second
 * argument. Previously each tab's `renderRow` called `beginEdit(r)`
 * from the Edit button, but `beginEdit` is only defined inside
 * EditableList's closure — `renderRow` is defined in the tab
 * component's own (different) scope, so that call threw
 * "ReferenceError: beginEdit is not defined" every time someone
 * clicked Edit. Passing it in as a parameter fixes that.
 *
 * The four file-import buttons in the header stay as REPLACE
 * (wholesale restore). The "Append from file" inside this page is the
...
interface EditableListProps<T> {
  ...
  /**
   * Body cells for a single row in view mode. Receives `beginEdit` so
   * each tab's Edit button can enter edit mode — `beginEdit` lives in
   * this component's closure, so it has to be passed in rather than
   * referenced from the row-renderer's own (different) scope.
   */
...
export default function DataManagerPage({ onBack }: { onBack: () => void }) {
  const {
    state,
    addGradeRow,
    updateGradeRow,
  ...
  return (
    <main className="flex-1 flex flex-col min-h-0 bg-slate-950">
      ...
      <footer className="px-6 py-3 border-t border-white/10 bg-slate-950/80 backdrop-blur-md text-[10px] uppercase tracking-wider font-bold text-slate-500 sticky bottom-0">
        {tab === 'grade'
...
function CatalogTab({
  ...
  const renderRow = (
    c: CatalogCourse,
    beginEdit: (row: CatalogCourse) => void
  ): ReactNode => (
    <>
...
function RosterTab({
  ...
  const renderRow = (
    r: RosterEntry,
    beginEdit: (row: RosterEntry) => void
  ): ReactNode => (
    <>
      <Td className="font-mono text-xs">{r.studentId}</Td>
